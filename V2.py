...
#------------> BEGIN OUTPUT 
   if (nt == math.trunc(nt / ntplot) * ntplot):
      ...
      plt.axis([0,r[-1]/1000,2*Href-A -100 ,A+100]) 
      ...
      plt.text(100, A-50,"t="+str((nt-ntplot)*dt/3600)+" hours")
      plt.text(100, A-100,"red: lower layer",color='red')
      plt.text(100, A-150,"blue: upper layer",color='blue')
      ...
      plt.text(100,0.7*2*A,"blue: upper layer",color='blue')
      plt.savefig(DirecOutputv+"GradWindAdjustment-Run"+str(Runnumber)+"-v"+str(nt)+".png")
      plt.close()

      #PLOT u
      plt.figure(figsize=(8,6))
      A = A = max([npy.amax(u[nt,0,:]),npy.amax(-u[nt,0,:]), npy.amax(u[nt,1,:]) , npy.amax(-u[nt,1,:])  ]) 
      plt.axis([0,rm[-1]/1000,-2*A,2*A])   
...
PE_ref2 = 0
for nx in range (0, nx_len):
   PE_F1 = PE_F1 + ro1*h[-1,0,nx]*g
   PE_F2 = PE_F2 + ro2*h[-1,1,nx]*g
   PE_ref1 = PE_ref1 + ro1*Href*g 
   PE_ref2 = PE_ref2 + ro2*Href*g

KE_F1 = 0
KE_F2 = 0
for nx in range (0,nx_len):
   KE_F1 = KE_F1 + ro1*(v[-1,0,nx]*v[-1,0,nx] + u[-1,0,nx]*u[-1,0,nx])
   KE_F2 = KE_F2 + ro2*(v[-1,1,nx]*v[-1,1,nx] + u[-1,1,nx]*u[-1,1,nx])

print(PE_ref1-PE_F1, PE_ref2-PE_F2, KE_F1, KE_F2 )
