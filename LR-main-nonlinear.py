...
a = [100000.0, 300000.0, 500000.0, 700000.0 , 900000.0]                  # horizontal scale of the forcing [m]
   ...
   for nt in range(len(time)):
      for nx in range(nx_len):
         if time[nt]<=T_forcing: 
            M[nt,0,nx] = -(Qmax/T_forcing) * math.exp(-math.pow((r[nx]-r0)/a[i],2))     	
            M[nt,1,nx] = (Qmax/(eps*T_forcing)) * math.exp(-math.pow((r[nx]-r0)/a[i],2)) 
         if time[nt]>T_forcing: 
            M[nt,0,nx] = 0.0
            M[nt,1,nx] = 0.0
